...
const AddOrUpdateUserModal: FC<AddUserModalProps> =({open = false, onClose, onSubmit, context})=> {
  ...
  return(
    <Modal open={open} onClose={onClose} onSubmit={onSubmit} aria-labelledby={"modal-title"}>
      <Box sx={ModalMixin}>
        <Typography id="modal-title" variant="h6" component="h2" sx={{paddingBottom: '15px'}}>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Controller
            ...
            render={({field: {name, value, onChange}})=>
              <TextField sx={TextFieldMixin} name={name} value={value} onChange={onChange} label={'Name'} fullWidth/>
            }
            name={'name'}
          />
          <Controller
            control={control}
            render={({field:  {name, value, onChange}})=>
              <TextField sx={TextFieldMixin} name={name} value={value} onChange={onChange} label={'Age'} fullWidth/>
            }
            name={'age'}
          />
        </form>
        <Box sx={{paddingTop: '10px'}}>
          <StandartButton type={'submit'}>{context.mode === 'add' ? 'Add user' : 'Update user'}</StandartButton>
